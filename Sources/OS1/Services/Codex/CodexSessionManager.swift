...
final class CodexSessionManager: ObservableObject {
    ...
    private func buildHeartbeatPrompt(session: CodexSession, availableCreds: [String] = []) -> String {
        ...
        10. Environment separation: dev/sandbox/staging work must use test resources only. Live production actions are allowed only in production mode and must be explicitly approved before execution.
