...
struct CompanyUnitEconomicsTests {
    ...
    func scaleAllowedWhenUnitEconomicsMeetThresholds() {
        ...
        let decision = CompanyLifecycleEngine.decide(
            snapshot(ledger: ledger, unitEconomics: report, experimentEvidence: strongExperimentEvidence())
        )
    ...
    private func strongExperimentEvidence() -> CompanyExperimentResult {
        CompanyExperimentResult(
            companyID: "company",
            experimentID: "pricing",
            templateKind: .pricing,
            cohortIDs: ["a"],
            sampleSize: 100,
