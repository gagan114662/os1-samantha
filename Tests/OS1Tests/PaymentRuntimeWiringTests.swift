...
struct PaymentRuntimeWiringTests {
    ...
    func stripeSandboxCheckoutClientCreatesMetadataRichCheckoutLink() async throws {
        ...
        let session = CompanyCheckoutSession(link: link, productID: "guide")
        #expect(session.provider == .stripe)
        #expect(session.checkoutURL == link.checkoutURL)
    }
    ...
    @Test
    func managerPersistsStripeProviderEventAndReconcilesLedger() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("payment-provider-event-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let worktree = root.appendingPathComponent("co", isDirectory: true)
        try FileManager.default.createDirectory(at: worktree, withIntermediateDirectories: true)
        try "[]".write(to: worktree.appendingPathComponent("LEDGER.json"), atomically: true, encoding: .utf8)
        ...
        let now = calendar.date(from: DateComponents(year: 2026, month: 5, day: 13, hour: 12))!
        let timestamp = Int(now.timeIntervalSince1970)
        let payload = Data(#"{"id":"evt_provider","type":"checkout.session.completed","amount_total":2900,"currency":"usd","payment_intent":"pi_provider","metadata":{"company_id":"co","utm_campaign":"co","utm_content":"post-1"}}"#.utf8)
        let header = PaymentWebhookReceiver.stripeSignatureHeader(
            payload: payload,
            timestamp: timestamp,
            endpointSecret: "whsec_provider"
        )
        let store = PaymentWebhookSeenEventStore(
            url: root.appendingPathComponent("seen.sqlite")
        )
        ...
        let result = try await MainActor.run {
            ...
            manager.replaceSessionsForTesting([session])
            CompanyAccessControl.grantCapabilities([.payments], companyID: "co")
            _ = try manager.recordStripeWebhook(
                companyID: "co",
                payload: payload,
                signatureHeader: header,
                endpointSecret: "whsec_provider",
            ...
            let providerEvents = try CompanyPaymentProviderEventStore(
                url: worktree.appendingPathComponent(CompanyPaymentProviderEventStore.fileName)
            ).load()
            let report = CompanyBookkeepingEngine.monthlyCompanyReport(
                companyID: "co",
                month: CompanyAccountingMonth.containing(now),
                ledger: manager.ledgerSummary(id: "co"),
                providerEvents: providerEvents
            )
            return (providerEvents, report.reconciliationLines)
        }

        #expect(result.0.count == 1)
        #expect(result.0.first?.kind == .charge)
        #expect(result.0.first?.sourceReference == "pi_provider")
        #expect(result.1.count == 1)
        #expect(result.1.first?.status == .matched)
    }
